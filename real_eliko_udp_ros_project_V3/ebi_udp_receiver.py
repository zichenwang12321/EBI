#!/usr/bin/env python3
import socket
import rospy
import os
import time
import csv
from std_msgs.msg import Float32MultiArray

# 初始化 CSV 路径
save_dir = os.path.expanduser("~/zichen/ebi_data_logs")
os.makedirs(save_dir, exist_ok=True)
csv_path = os.path.join(save_dir, "ebi_udp_log.csv")

# UDP 配置
UDP_IP = "192.168.1.134"
UDP_PORT = 1234

# 全局缓存
data_buffer = []
seq_counter = 0

def save_to_csv(buffer):
    if not buffer:
        return
    with open(csv_path, 'a', newline='') as f:
        writer = csv.writer(f, delimiter=';')
        for entry in buffer:
            writer.writerow(entry)

def ebi_data_udp_receiver():
    global seq_counter, data_buffer

    rospy.init_node('ebi_udp_listener', anonymous=True)
    pub = rospy.Publisher('/eliko/ebi_data', Float32MultiArray, queue_size=10)

    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.bind((UDP_IP, UDP_PORT))
    rospy.loginfo("Listening for EBI UDP data on %s:%d", UDP_IP, UDP_PORT)

    if not os.path.exists(csv_path):
        with open(csv_path, 'w', newline='') as f:
            writer = csv.writer(f, delimiter=';')
            writer.writerow(['seq', 'timestamp', 'data'])

    last_save_time = time.time()

    while not rospy.is_shutdown():
        try:
            data, _ = sock.recvfrom(4096)
            decoded = data.decode('utf-8').strip()

            parts = decoded.split(';')
            if len(parts) < 3:
                rospy.logwarn("Malformed packet: %s", decoded)
                continue

            seq = int(parts[0])
            timestamp = float(parts[1])
            lines = parts[2:]  # 剩余是数据

            parsed_data = []
            for line in lines:
                line = line.strip()
                if not line:
                    continue
                elements = line.split('   ')
                for elem in elements:
                    elem = elem.strip()
                    if not elem:
                        continue
                    if '+' in elem:
                        real, imag = elem.split(' + ')
                        imag = imag.replace('i', '')
                        parsed_data.append(float(real))
                        parsed_data.append(float(imag))
                    elif '-' in elem[1:]:
                        real, imag = elem.split(' - ', 1)
                        imag = imag.replace('i', '')
                        parsed_data.append(float(real.replace(' ','')))
                        parsed_data.append(-float(imag))

            if not parsed_data:
                rospy.logwarn("No valid data parsed from packet")
                continue

            msg = Float32MultiArray()
            msg.data = parsed_data
            pub.publish(msg)

            data_buffer.append([seq, timestamp, parsed_data])
            seq_counter += 1

            now = time.time()
            if now - last_save_time >= 1.0:
                save_to_csv(data_buffer)
                data_buffer = []
                last_save_time = now

        except Exception as e:
            rospy.logwarn("Error processing packet: %s", str(e))
            continue

if __name__ == '__main__':
    try:
        ebi_data_udp_receiver()
    except rospy.ROSInterruptException:
        pass
